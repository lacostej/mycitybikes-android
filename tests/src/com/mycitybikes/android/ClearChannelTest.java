...
public class ClearChannelTest extends AndroidTestCase {
	...
	private final String OFFLINE_STATION_STATUS = 
	...
	private final String ONLINE_STATION_STATUS = 
		"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
     + "<string xmlns=\"http://smartbikeportal.clearchannel.no/public/mobapp/\">"
	...
	public void testLoadOsloBikeLocations() throws IOException {
		...
		assertEquals("we skip comments and loaded proper locations", 2, bikeLocations.size());
	...
	*/
	...
	public void testLoadOsloBikeLocations() throws IOException {
		...
		assertEquals(59.92786125852981, bikeLocations.get(0).getLatitude());
		assertEquals(10.709009170532226, bikeLocations.get(0).getLongitude());
		assertEquals("Middelthunsgate (vis-a-vis nr. 21, retning Kirkeveien)", bikeLocations.get(0).getDescription());
		
		assertEquals("trim works", "Prinsensgt. (Kirkegt.)", bikeLocations.get(2).getDescription());
		
	}

	private ByteArrayInputStream getStringInputStream(String string) throws UnsupportedEncodingException {
		return new ByteArrayInputStream(string.getBytes("UTF-8"));
	}

	public void testParseOfflineStatus() throws IOException {
		BikeStationStatus status = ClearChannel.parseStatus(getStringInputStream(OFFLINE_STATION_STATUS));
		assertEquals("67-Holtegt. 20 (krysset Uranienborgvn)", status.getDescription());
	...
	public void testParseOnlineStatus() throws IOException {
		BikeStationStatus status = ClearChannel.parseStatus(getStringInputStream(ONLINE_STATION_STATUS));
		assertEquals("10-Kirkeristen Øst (mot Storgt.)", status.getDescription());
	...
	public void testFetchLiveStatus() {
		...
		Log.i(Constants.TAG, "Fetch status for station #" + id + ": " + status.toString());
	}
}
